...
public class MatchAwardDataDocumentTests
{
    ...
    public void GetElementObjects_WithItems_ReturnsAllElementsAsObjects()
    {
        ...
        // act
        List<object> result = [.. matchAwardData.GetElementObjects()];

        // assert
        result.Should().HaveCount(2);
        result.Should().AllBeOfType<MatchAward>();
        result.OfType<MatchAward>().Should().Contain(m => m.Id == "EndOfMatchAwardMVPBoolean");
        result.OfType<MatchAward>().Should().Contain(m => m.Id == "EndOfMatchAwardMostXPContributionValue");
    }
    ...
    public void GetElementObjects_WithItems_ReturnsObjectsWithCorrectProperties()
    {
        ...
        MatchAward mostXP = result.OfType<MatchAward>().First(m => m.Id == "EndOfMatchAwardMostXPContributionValue");
        MostXPBasicAssertions(mostXP);
    }
